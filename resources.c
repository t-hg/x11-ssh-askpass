...
Bool get_boolean_resource(char *res_name, char *res_class, Bool default_value) {
  char *tmp, buf[100];
...
int get_integer_resource(char *res_name, char *res_class, int default_value) {
  int val;
...
double get_float_resource(char *res_name, char *res_class) {
  double val;
  char c, *s = get_string_resource(res_name, res_class);
  if (!s)
    return 0.0;
...
unsigned int get_pixel_resource(char *res_name, char *res_class, Display *dpy,
                                Colormap cmap, unsigned int default_value) {
  XColor color;
  char *s = get_string_resource(res_name, res_class);
  char *s2;
  if (!s)
