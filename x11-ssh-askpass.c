...
XftFont *getFontResource(AppInfo *app, char *instanceName, char *className) {
  ...
  if (!f) {
    if (!(f = XftFontOpenName(app->dpy, DefaultScreen(app->dpy),
                              fallbackFont))) {
      fprintf(stderr, "error, cannot load font from name: '%s'\n",
              fallbackFont);
      return NULL;
...
void createDialog(AppInfo *app) {
  ...
  labelColor = getStringResourceWithDefault("dialog.foreground",
  ...
  labelColor = getStringResourceWithDefault("okButton.foreground",
  ...
  labelText = getStringResourceWithDefault("cancelButton.label", "Button.Label",
                                           "Cancel");
  labelColor = getStringResourceWithDefault("cancelButton.foreground",
                                            "Button.Foreground", "#000000");
  ...
  d->indicator.w3.borderColor =
      get_pixel_resource("indicator.borderColor", "Indicator.BorderColor",
                         app->dpy, app->colormap, app->black);
  d->indicator.borderColorWhenLit = get_pixel_resource(
      "indicator.borderColorWhenLit", "Indicator.BorderColorWhenLit", app->dpy,
      app->colormap, d->indicator.w3.borderColor);
...
void paintLabel(AppInfo *app, Drawable draw, LabelInfo label) {
  ...
  while (NULL != t) {
    if (t->text) {
      XftColor *color;
      if (!XftColorAllocName(app->dpy, DefaultVisualOfScreen(app->screen),
                             app->colormap, label.color, color)) {
        fprintf(stderr, "error, cannot allocate color '%s'", label.color);
      };
      XftDraw *d = XftDrawCreate(
          app->dpy, draw, DefaultVisualOfScreen(app->screen), app->colormap);
      XftDrawStringUtf8(d, color, label.font, x, y, (XftChar8 *)t->text,
                        t->textLength);
      if (d) {
        XftDrawDestroy(d);
      }
      if (color) {
        XftColorFree(app->dpy, DefaultVisualOfScreen(app->screen),
                     DefaultColormapOfScreen(app->screen), color);
      }
    }
    y += t->descent;
    t = t->next;
